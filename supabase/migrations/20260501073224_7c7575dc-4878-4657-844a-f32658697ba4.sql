...
BEGIN
  ...
  -- has arrived, when the owning rule is auto-post + fixed-amount. Catches
  -- occurrences materialised earlier as 'pending' (look-ahead horizon, or
  -- before the rule was switched to auto_post).
  -- NOTE: alias the rules table as `rr` (not `r`) so it doesn't clash with
  -- the PL/pgSQL record variable `r` declared above — referencing `r.*` in
  -- the SELECT against the unassigned record raises:
  --   "record \"r\" is not assigned yet"
  FOR o IN
    SELECT occ.id AS occ_id, occ.effective_on AS occ_effective_on, rr.*
  ...
  LOOP
    ...
    VALUES
      (v_uid, o.occ_effective_on, o.amount, o.type, o.source_account_id, o.destination_account_id, o.category_id, o.description, o.note, o.id)
