...
- Period series is generated by the exact same generator as execution dates, substituting `PeriodDayRule` / `PeriodDayOfMonth`, with the same "skip anchors strictly before StartDate" filter. The nth execution is paired 1:1 with the nth period.
...
Scenario C is the canonical FirstDay example under Option 1:
- StartDate 10.05.26, Interval 3, PeriodRule FirstDay, execs 15.05, 15.08, 15.11
- Base period series (FirstDay from 10.05, skip 01.05) → 01.06, 01.09, 01.12
- Off 0: exec#1 → 01.06–31.08; exec#2 → 01.09–30.11
- Off +1: exec#1 → 01.09–30.11; exec#2 → 01.12.26–28.02.27
- Off −2: exec#1 → 01.12.25–28.02.26; exec#2 → 01.03–31.05.26
...
- `series_next(anchor date, rule day_rule_v2, dom smallint, interval_months smallint, min_date date) → date` — core primitive. Steps `interval_months` at a time from `anchor`'s month, applies rule, returns first result `>= min_date`. Handles FixedDay 29/30/31 clamp.
- `execution_dates(rule_id, from_date, to_date) → SETOF date` — pre-weekend due dates.
- `weekend_shift(d date, adj weekend_adjust_v2) → date`.
- `period_bounds(rule_id, exec_due date) → (from date, to date)` — finds the n such that `execution_dates` n-th element = `exec_due`, applies `series_next` with period params for the same n, adds `PeriodOffset * interval_months`, computes `to = next_period_start − 1 day`.
